  ...
  persist(
    (set, get) => ({
      ...
      addNode: (opcode, workflowName = "main") => {
        ...
        console.log(
          "[addNode] Found nodes at line:",
          nodesLineIndex,
          "indent:",
          nodesIndent,
        );
        console.log(
          "[addNode] Last node ends at:",
          lastNodeEndIndex,
          "indent:",
          lastNodeIndent,
        );
        ...
        if (nodesLineIndex === -1) {
          console.warn(
            `nodes: section not found for workflow "${workflowName}"`,
          );
          return null;
        }
        ...
        console.log(
          "[addNode] Calling setSource with new source length:",
          newSource.length,
        );
        state.setSource(newSource);
      ...
      duplicateNode: (nodeId) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (match && match[2] === nodeId) {
            ...
            nodeIndent = match[1].length;
            console.log(
              "[duplicateNode] Found node at line:",
              i,
              "indent:",
              nodeIndent,
            );
        ...
        console.log(
          "[duplicateNode] Calling setSource, new length:",
          newSource.length,
        );
        state.setSource(newSource);
        set({ selectedNodeId: newId });
      ...
      updateNodeInput: (nodeId, inputKey, newValue) => {
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[updateNodeInput] Node "${nodeId}" not found in source`,
          );
          return false;
        }
      ...
      connectNodes: (fromNodeId, toNodeId) => {
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[connectNodes] Node "${fromNodeId}" not found in source`,
          );
          return false;
        }
      ...
      connectBranch: (fromNodeId, toNodeId, branchLabel) => {
        console.log(
          "[connectBranch] Connecting branch",
          branchLabel,
          "from",
          fromNodeId,
          "to",
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[connectBranch] Node "${fromNodeId}" not found in source`,
          );
          return false;
        }
        ...
        if (inputsLine === -1) {
          console.warn(
            `[connectBranch] No inputs: section found for node "${fromNodeId}"`,
          );
          return false;
        }
        ...
        if (branchLabelLine !== -1) {
          ...
          if (isCatchBranch) {
            ...
            if (bodyLine !== -1) {
              ...
              const bodyIndent =
                lines[bodyLine].match(/^(\s*)/)?.[1].length || 0;
              ...
              while (bodySearchLine < lines.length) {
                ...
                if (branchMatch) {
                  lines[bodySearchLine] =
                    `${branchMatch[1]}branch: ${toNodeId}`;
              ...
              lines.splice(
                bodyLine + 1,
                0,
                `${bodyBranchIndent}branch: ${toNodeId}`,
              );
              state.setSource(lines.join("\n"));
              ...
              state.setSource(lines.join("\n"));
              console.log(
                "[connectBranch] Inserted body with branch for CATCH",
              );
              return true;
            }
          } else {
      ...
      disconnectNode: (nodeId) => {
        ...
        while (nodeEndLine < lines.length) {
          ...
          if (line.match(/^\s+next:\s*/)) {
            const indent =
              line.match(/^(\s+)next:/)?.[1] || " ".repeat(nodeIndent + 2);
        ...
        console.log(
          "[disconnectNode] No next: line found, node already disconnected",
        );
        return true;
      },
      ...
      convertOrphanToReporter: (
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[convertOrphanToReporter] Target node "${targetNodeId}" not found in source`,
          );
          return false;
        }
        ...
        console.log(
          "[convertOrphanToReporter] Successfully converted orphan to reporter",
        );
        return true;
      },
      ...
      deleteReporter: (parentNodeId, inputPath) => {
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[deleteReporter] Parent node "${parentNodeId}" not found`,
          );
          return false;
        }
      ...
      updateWorkflowInterface: (workflowName, inputs, outputs) => {
        console.log(
          "[updateWorkflowInterface] workflowName:",
          workflowName,
          "inputs:",
          inputs,
          "outputs:",
        ...
        if (interfaceStartLine === -1) {
          console.warn(
            `[updateWorkflowInterface] Interface section not found for workflow "${workflowName}"`,
          );
          return false;
        }
      ...
      addVariable: (workflowName, name, defaultValue) => {
        ...
        if (variablesLine === -1) {
          console.warn(
            `[addVariable] Variables section not found for workflow "${workflowName}"`,
          );
          return false;
        }
      ...
      updateVariable: (workflowName, oldName, newName, newValue) => {
        ...
        if (varLine === -1) {
          console.warn(
            `[updateVariable] Variable "${oldName}" not found in workflow "${workflowName}"`,
          );
          return false;
        }
      ...
      disconnectConnection: (fromNodeId, toNodeId, branchLabel) => {
        console.log(
          "[disconnectConnection] Disconnecting",
          fromNodeId,
          "->",
          toNodeId,
          "branch:",
        ...
        if (nodeStartLine === -1) {
          console.warn(
            `[disconnectConnection] Node "${fromNodeId}" not found in source`,
          );
          return false;
        }
        ...
        if (inputsLine === -1) {
          console.warn(
            `[disconnectConnection] No inputs: section found for node "${fromNodeId}"`,
          );
          return false;
        }
        ...
        if (branchLabelLine === -1) {
          console.warn(
            `[disconnectConnection] Branch label "${branchLabel}" not found in inputs`,
          );
          return false;
        }
            ...
            if (bodyMatch) {
              ...
              while (bodyLine < lines.length) {
                ...
                if (branchMatch && branchMatch[2] === toNodeId) {
                  ...
                  state.setSource(lines.join("\n"));
                  console.log(
                    "[disconnectConnection] Disconnected CATCH branch",
                  );
                  return true;
                }
                bodyLine++;
        ...
        console.warn(
          `[disconnectConnection] Could not find branch reference to "${toNodeId}"`,
        );
        return false;
      },
      ...
      deleteVariable: (workflowName, name) => {
        ...
        if (varLine === -1) {
          console.warn(
            `[deleteVariable] Variable "${name}" not found in workflow "${workflowName}"`,
          );
          return false;
        }
        ...
        const newLines = [
          ...lines.slice(0, varLine),
          ...lines.slice(varLine + 1),
        ];
        state.setSource(newLines.join("\n"));
