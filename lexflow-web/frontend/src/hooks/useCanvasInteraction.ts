...
import { findNearestPortFromRegistry, toYamlNodeId, getWorkflowNameFromSlotId } from "../utils/wireUtils";
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (draggingWire) {
      if (draggingWire.nearbyPort) {
        // Convert registry IDs to YAML IDs (handles start-* and workflowName::nodeId formats)
        const sourceId = toYamlNodeId(draggingWire.sourceNodeId);
        const targetId = toYamlNodeId(draggingWire.nearbyPort.nodeId);

        // Get workflow name from slot IDs (handles both start-* and workflowName::nodeId)
        const sourceWorkflow = getWorkflowNameFromSlotId(draggingWire.sourceNodeId);
        const targetWorkflow = getWorkflowNameFromSlotId(draggingWire.nearbyPort.nodeId);
        ...
        if (draggingWire.branchLabel) {
          ...
          // Regular: output to input - use source workflow
          connectNodes(sourceId, targetId, sourceWorkflow);
        } else {
          // Reverse: input to output - use target workflow
