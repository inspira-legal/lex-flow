...
export function workflowToTree(workflowData: WorkflowData): WorkflowTree | { error: string } {
  ...
  return { type: 'project', workflows: workflowTrees, interface: mainInterface }
}
...
function buildWorkflowTree(workflow: WorkflowDefinition): WorkflowNode {
  ...
  const potentialOrphanIds = new Set([...allNodeIds].filter(id => !allVisited.has(id)))
  ...
  const orphanIds = new Set([...allNodeIds].filter(id => !allVisited.has(id)))
  ...
  for (const headId of orphanHeads) {
    ...
    while (currentOrphanId && !processedOrphans.has(currentOrphanId)) {
      ...
      if (node.isReporter) {
        ...
        currentOrphanId = node.next && orphanIds.has(node.next) ? node.next : undefined
        continue
      }
...
function nodeToTree(
  ...
  if (['control_for', 'control_foreach', 'control_while'].includes(opcode)) {
    ...
    const bodyBranch = typeof bodyInput === 'object' ? bodyInput.branch : undefined
    if (bodyBranch) {
      treeNode.children.push(buildBranch('BODY', bodyBranch, allNodes, allVisited))
    }
  } else if (['control_if', 'control_if_else'].includes(opcode)) {
    const thenInput = (inputs['THEN'] || inputs['then'] || {}) as InputValue
    const thenBranch = typeof thenInput === 'object' ? thenInput.branch : undefined
    if (thenBranch) {
      treeNode.children.push(buildBranch('THEN', thenBranch, allNodes, allVisited))
    }

    const elseInput = (inputs['ELSE'] || inputs['else'] || {}) as InputValue
    const elseBranch = typeof elseInput === 'object' ? elseInput.branch : undefined
    if (elseBranch) {
      treeNode.children.push(buildBranch('ELSE', elseBranch, allNodes, allVisited))
    }
...
function formatInputs(
  ...
  for (const [key, value] of Object.entries(inputs)) {
    ...
    if (['BODY', 'body', 'THEN', 'then', 'ELSE', 'else', 'TRY', 'FINALLY'].includes(key)) {
      continue
    }
...
function formatValue(
  ...
  if (typeof value === 'object' && value !== null) {
      ...
      return {
        ...
        inputs: Object.fromEntries(
          Object.entries(nodeInputs)
            .filter(([k]) => !k.startsWith('CATCH') && !['BODY', 'THEN', 'ELSE'].includes(k))
    ...
    if ('workflow_call' in inputValue) {
      return { type: 'workflow_call', name: inputValue.workflow_call as string }
    }
...
function extractLoopConfig(
  ...
  if (opcode === 'control_for') {
    ...
    config.start = getRawValue(inputs['START'] ?? inputs['start'] ?? 0) as number
    ...
    config.var = getRawValue(inputs['VAR'] ?? inputs['var'] ?? 'item') as string
    config.iterable = formatValue(inputs['ITERABLE'] ?? inputs['iterable'] ?? [], allNodes)
  } else if (opcode === 'control_while') {
    config.condition = formatValue(inputs['CONDITION'] ?? inputs['condition'] ?? true, allNodes)
  }
...
function getRawValue(value: unknown): unknown {
  if (typeof value === 'object' && value !== null && 'literal' in (value as Record<string, unknown>)) {
...
function extractForkBranches(
  ...
  const branchList = (inputs['BRANCHES'] ?? inputs['branches'] ?? []) as InputValue[]
  if (Array.isArray(branchList) && branchList.length > 0) {
    branchList.forEach((branchRef, i) => {
      const branchId = typeof branchRef === 'object' && branchRef !== null
        ? (branchRef as InputValue).branch
        : undefined
      if (branchId) {
        branches.push(buildBranch(`BRANCH${i + 1}`, branchId, allNodes, allVisited))
      }
    })
  } else {
    // Try individual keys
    ...
    while (`BRANCH${i}` in inputs) {
      ...
      const branchId = typeof branchRef === 'object' && branchRef !== null
        ...
        : undefined
      if (branchId) {
        branches.push(buildBranch(`BRANCH${i}`, branchId, allNodes, allVisited))
      }
      i++
    }
  }
...
function extractTryBranches(
  ...
  // TRY body
  const tryInput = (inputs['TRY'] ?? inputs['BODY'] ?? inputs['body'] ?? {}) as InputValue
  ...
  // CATCH handlers
  const handlers = (inputs['HANDLERS'] ?? inputs['handlers'] ?? []) as CatchHandler[]
  if (Array.isArray(handlers) && handlers.length > 0) {
    handlers.forEach((handler, i) => {
      if (typeof handler === 'object' && handler !== null) {
        const handlerBranch = handler.branch
        if (handlerBranch) {
          const branch = buildBranch(`CATCH${i + 1}`, handlerBranch, allNodes, allVisited)
    ...
    while (`CATCH${i}` in inputs) {
      ...
      if (typeof catchInput === 'object' && catchInput !== null) {
        ...
        const handlerBranch = typeof bodyInfo === 'object' && bodyInfo !== null
          ? (bodyInfo as InputValue).branch
          : undefined
        if (handlerBranch) {
          const branch = buildBranch(`CATCH${i}`, handlerBranch, allNodes, allVisited)
  ...
  // FINALLY
  const finallyInput = (inputs['FINALLY'] ?? inputs['finally'] ?? {}) as InputValue
  const finallyBranch = typeof finallyInput === 'object' ? finallyInput.branch : undefined
  if (finallyBranch) {
