...
function getTargetWorkflow(e: MouseEvent): string {
  // Select the main canvas SVG using the data attribute
  const svg = document.querySelector('[data-canvas-container] > svg')
  if (!svg) return "main"
...
export function DragPreview() {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      const isOverCanvas =
        target.closest("svg") !== null || target.closest('[class*="canvas"]') !== null
  ...
  }, [draggingWorkflowCall, setDraggingWorkflowCall, addWorkflowCallNode, openNodeEditor])
  ...
  if (draggingWorkflowCall) {
    return (
      <div
        className={cn(previewVariants())}
        style={{
          left: position.x + 16,
          top: position.y + 16,
        }}
      >
        <div className={cn(cardVariants({ type: "workflow" }))}>
          <span className={cn(nameVariants())}>{draggingWorkflowCall.workflowName}</span>
          <span className={cn(opcodeVariants())}>workflow_call</span>
        </div>
        <div className={cn(hintVariants())}>Drop on canvas to add</div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className={cn(cardVariants({ type: "opcode" }))}>
        <span className={cn(nameVariants())}>{displayName}</span>
        <span className={cn(opcodeVariants())}>{draggingOpcode.name}</span>
      </div>
      <div className={cn(hintVariants())}>Drop on canvas to add</div>
    </div>
  )
