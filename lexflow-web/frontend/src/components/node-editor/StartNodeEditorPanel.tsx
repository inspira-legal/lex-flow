...
export function StartNodeEditorPanel({ workflowName, onClose }: StartNodeEditorPanelProps) {
  const { tree, addVariable, updateVariable, deleteVariable, updateWorkflowInterface } = useWorkflowStore()
  const { selectStartNode } = useUiStore()

  // Find the workflow
  const workflow = tree?.workflows.find((w) => w.name === workflowName) as WorkflowNode | undefined
  ...
  const handleAddInput = () => {
    ...
    if (updateWorkflowInterface(workflowName, newInputs, workflow.interface.outputs)) {
      setNewInput('')
    }
  }
  ...
  const handleRemoveInput = (input: string) => {
    ...
    updateWorkflowInterface(workflowName, newInputs, workflow.interface.outputs)
  }

  const handleAddOutput = () => {
    if (!newOutput.trim()) return
    const newOutputs = [...workflow.interface.outputs, newOutput.trim()]
    if (updateWorkflowInterface(workflowName, workflow.interface.inputs, newOutputs)) {
      setNewOutput('')
    }
  }
  ...
  const handleRemoveOutput = (output: string) => {
    ...
    updateWorkflowInterface(workflowName, workflow.interface.inputs, newOutputs)
  }
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.nodeInfo}>
          <div className={styles.colorBar} style={{ backgroundColor: '#22C55E' }} />
        ...
        <div className={styles.section}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <input
              ...
              style={{ flex: 1 }}
            />
            <button className={styles.actionBtn} onClick={handleAddInput} style={{ flex: 'none', width: 'auto' }}>
              Add
            </button>
          </div>
        </div>
        ...
        <div className={styles.section}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <input
              ...
              style={{ flex: 1 }}
            />
            <button className={styles.actionBtn} onClick={handleAddOutput} style={{ flex: 'none', width: 'auto' }}>
              Add
            </button>
          </div>
        </div>
                  ...
                  {editingVar === name ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && handleSaveEditVar()}
                      />
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button
                    ...
                    >
                      ...
                      <span className={styles.valueContent}>{formatValue(value)}</span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
