...
import { PlayIcon, StopIcon, StepForwardIcon, TrashIcon, CloseIcon } from "@/components/icons"
    ...
    >
      ...
      <div className={headerVariants()}>
        <div className={titleGroupVariants()}>
          ...
          <span className={statusDotVariants({ status: isExecuting ? "running" : executionError ? "error" : "idle" })} />
        </div>
        <div className={actionsVariants()}>
          ...
          <button className={iconBtnVariants()} title="Step forward (coming soon)" disabled>
            <StepForwardIcon />
          </button>
          <button
          ...
          <div className={separatorVariants()} />
          <button className={closeBtnVariants()} onClick={toggleExecutionPanel} title="Close panel">
            <CloseIcon />
          </button>
        </div>
      </div>
      ...
      <div className={panesVariants()}>
        ...
        {showPreview && showConsole && <div className={paneResizerVariants()} />}
                ...
                <>
                  {executionOutput && (
                    <pre ref={outputRef} className={outputTextVariants()}>
                      {executionOutput}
                      {isExecuting && <span className={cursorVariants()}>|</span>}
                    </pre>
                  )}
...
function InputField({ name }: { name: string }) {
  ...
  return (
    <div className={inputFieldVariants()}>
      <label className={inputFieldLabelVariants()}>{name}:</label>
      <input
        type="text"
        value={String(value)}
        onChange={(e) => {
