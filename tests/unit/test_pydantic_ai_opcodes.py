...
try:
    from lexflow_opcodes.ai import (
        _normalize_messages,
...
class TestPydanticAIOpcodes:
    ...
    async def test_create_vertex_model_basic(self):
        ...
        with (
            patch(
                "lexflow_opcodes.ai.GoogleProvider"
            ) as mock_provider_class,
            patch(
                "lexflow_opcodes.ai.GoogleModel"
            ) as mock_model_class,
        ):
            mock_provider = Mock()
    ...
    async def test_create_agent_minimal(self):
        ...
        with patch("lexflow_opcodes.ai.Agent") as mock_agent_class:
            mock_model = Mock()
            mock_agent = Mock()
...
class TestAiAgentWithToolsOpcode:
    ...
    async def test_basic_call_structure(self):
        ...
        with (
            patch("lexflow_opcodes.ai.Agent") as MockAgent,
            patch("lexflow_opcodes.ai.Tool") as MockTool,
        ):
...
class TestAiAgentWithToolsEdgeCases:
    ...
    async def test_empty_tools_list(self):
        ...
        with patch("lexflow_opcodes.ai.Agent") as MockAgent:
            mock_result = MagicMock()
